...
from .models import Exam, ExamBooklet, ExamAttempt, ExamResult, ExamBookletResult
...
class ExamBookletInline(admin.TabularInline):
    ...
    readonly_fields = ("end_question_display",)
    ordering = ("order",)
    ...
    end_question_display.short_description = "شماره پایان"
...
class ExamAdmin(admin.ModelAdmin):
    ...
    def finalize_view(self, request, object_id):
        ...
        if not self.has_change_permission(request, exam):
            ...
            return HttpResponseRedirect(
                reverse("admin:exams_exam_change", args=[exam.pk])
            )

        try:
            result = finalize_exam(exam_id=exam.id)
            self.message_user(
                request,
                (
            ...
            self.message_user(
                request,
                f"در فرایند تصحیح و رتبه‌بندی خطایی رخ داد: {error}",
                level=messages.ERROR,
            )
...
class ExamBookletAdmin(admin.ModelAdmin):
    list_display = (
        "exam", "title", "subject", "order",
        "start_question", "question_count",
        "end_question_display",
    )
    list_filter = ("subject", "exam")
    search_fields = ("exam__title", "title", "subject")
    ordering = ("exam", "order")
    readonly_fields = ("end_question_display",)
...
class ExamAttemptAdmin(admin.ModelAdmin):
    list_display = (
        "user", "exam", "status", "started_at",
        "expires_at", "submitted_at", "score", "raw_score",
    )
    list_filter = ("status", "exam")
    search_fields = ("user__username", "exam__title")
    readonly_fields = (
        "user", "exam", "started_at", "expires_at",
        "submitted_at", "status", "answers",
        "score", "raw_score",
        "correct_count", "wrong_count",
        "unanswered_count", "created_at", "updated_at",
    )
...
class ExamResultAdmin(admin.ModelAdmin):
    list_display = (
        "user", "exam", "is_final",
        "score", "raw_score",
        "national_rank", "provincial_rank",
        "finalized_at",
    )
    list_filter = ("is_final", "exam")
    search_fields = ("user__username", "exam__title")
...
class ExamBookletResultAdmin(admin.ModelAdmin):
    list_display = (
        "result", "booklet",
        "score", "raw_score", "decile",
        "national_rank", "provincial_rank",
    )
    list_filter = ("booklet", "decile")
    search_fields = (
