...
class ExamBookletSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ExamBooklet
        fields = [
            "id", "title", "subject", "order",
            "start_question", "question_count",
            "end_question", "question_range",
        ]

    def get_question_range(self, obj):
        if obj.question_count <= 0:
            return ""
        return f"{obj.start_question} تا {obj.end_question}"
...
class ExamListSerializer(serializers.ModelSerializer):
    ...
    booklets = ExamBookletSerializer(many=True, read_only=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            "total_questions", "booklet_count", "booklets",
            "questions_pdf_url", "answer_pdf_url",
            "status", "is_active",
            "has_attempt", "attempt_id", "attempt_status",
            "result_available", "can_start",
        ]
    ...
    def get_has_attempt(self, obj):
        return self.get_current_attempt(obj) is not None

    def get_attempt_id(self, obj):
        attempt = self.get_current_attempt(obj)
    ...
    def get_can_start(self, obj):
        attempt = self.get_current_attempt(obj)
        if attempt:
            return attempt.status == ExamAttemptStatus.IN_PROGRESS
        return obj.get_status() == "running"
...
class ExamBookletResultSerializer(serializers.ModelSerializer):
    booklet_title = serializers.CharField(source="booklet.title", read_only=True)
    subject = serializers.CharField(source="booklet.subject", read_only=True)
    booklet_order = serializers.IntegerField(source="booklet.order", read_only=True)

    class Meta:
        model = ExamBookletResult
        fields = [
            "id", "booklet", "booklet_title", "subject", "booklet_order",
            "total_questions", "correct_count", "wrong_count",
            "unanswered_count", "score", "raw_score",
            "decile", "national_rank", "national_participants",
            "provincial_rank", "provincial_participants",
            "national_average", "provincial_average",
            "performance_title", "performance_message",
        ]
        read_only_fields = fields
...
class ExamResultSerializer(serializers.ModelSerializer):
    ...
    exam_category_label = serializers.CharField(
        source="exam.get_category_display", read_only=True
    )
    username = serializers.CharField(source="user.username", read_only=True)
    user_first_name = serializers.CharField(source="user.first_name", read_only=True)
    user_last_name = serializers.CharField(source="user.last_name", read_only=True)
    user_province = serializers.CharField(source="user.province", read_only=True)
    booklet_results = ExamBookletResultSerializer(many=True, read_only=True)

    class Meta:
        model = ExamResult
        fields = [
            "id", "exam", "exam_title", "exam_category", "exam_category_label",
            "user", "username", "user_first_name", "user_last_name",
            "user_province", "is_final", "finalized_at",
            "total_questions", "correct_count", "wrong_count",
            "unanswered_count", "score", "raw_score",
            "national_rank", "national_participants",
            "provincial_rank", "provincial_participants",
            "booklet_results", "created_at", "updated_at",
        ]
        read_only_fields = fields
